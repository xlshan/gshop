            ...
            <div class="content">
              ...
              <div class="recommend">
                <span class="iconfont" :class="
                    r.rateType === 0 ? 'icon-thumb_up' : 'icon-thumb_down'
                  "></span>
                <span class="item" v-for="(item, index) in r.recommend" :key="index">{{ item }}</span>
              </div>
              <div class="time">{{ r.rateTime | date-format }}</div>
            </div>
          </li>
        </ul>
      </div>
  ...
  computed: {
    ...
    filterRating () {
      ...
      return rating.filter(item => {
        // item.rateType==0 满意 this.selectType
        // item.rateType==1 不满意 this.selectType
        // item.rateType==2 全部 this.selectType
